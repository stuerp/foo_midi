...
/** $VER: FMMPlayer.cpp (2025.06.16) - Wrapper for juno's fmmidi **/
...
bool FMMPlayer::Startup()
{
    ...
    if (_tfopen_s(&fp, pfc::stringcvt::string_os_from_utf8(_ProgramPath.c_str()), _T("rt")) != 0)
    ...
    while (!::feof(fp))
    {
        ...
        if (c == '*')
        {
            ...
            if (::fscanf_s(fp, "%d%d%d%d%d%d%d", &prog, &p.ALG, &p.FB, &p.LFO, &p.key, &p.panpot, &p.assign) == 7 &&
                ...
                ::fscanf_s(fp, "%d%d%d%d%d%d%d%d%d%d", &p.op4.AR, &p.op4.DR, &p.op4.SR, &p.op4.RR, &p.op4.SL, &p.op4.TL, &p.op4.KS, &p.op4.ML, &p.op4.DT, &p.op4.AMS) == 10)
            {
                _Factory->set_drum_program(prog, p);
            }
        }
    }
...
void FMMPlayer::SetProgramPath(const char * path)
{
    _ProgramPath = pfc::io::path::combine(path, "Programs.txt");
}
