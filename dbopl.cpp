...
#define REGCHAN(_FUNC_)                                                        \
	...
	void Chip::Setup(Bit32u rate) {
         ...
         */
			static const Bit32u precalculated_table[62] = { 2152, 2700, 3228, 3712, 4304, 5399, 6456, 7424, 8608, 10799, 12912, 14849, 17216, 21598,
			...
			static const Bit32u precalculated_table[62] = { 2342, 2939, 3513, 4040, 4685, 5877, 7027, 8081, 9369, 11754, 14054, 16162, 18738, 23508,
			...
			static const Bit32u precalculated_table[62] = { 4685, 5877, 7027, 8081, 9369, 11754, 14054, 16162, 18738, 23508, 28108, 32325, 37478,
			...
			static const Bit32u precalculated_table[62] = { 2078, 2607, 3117, 3584, 4156, 5213, 6233, 7168, 8311, 10426, 12466, 14336, 16621, 20853,
				                                            24933, 28672, 33243, 41705, 49865, 57344, 66486, 83410, 99729, 114688, 132972, 166842, 199490,
				                                            229376, 265986, 333764, 399073, 458752, 524288, 655360, 786432, 917504, 1048576, 1310720,
				                                            1572864, 1835008, 2097152, 2621440, 3145728, 3670016, 4194304, 5242880, 6291456, 7340032,
				                                            8388608, 10485760, 12582912, 14680064, 16777216, 20971520, 25165824, 29360128, 33554432,
				                                            41943040, 50331648, 58720256, 67108864, 67108864 };
			for(Bit8u i = 0; i < 62; i++)
				attackRates[i] = precalculated_table[i];
		}
	...
	void Handler::Generate(Bit32s* buffer,
	                       Bitu samples) {
		Bit32s temp[512];
		if(!chip.opl3Active) {
			while(samples) {
