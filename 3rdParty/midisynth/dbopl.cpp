...
/*
 *  Modified for midisynth to only render the first two channels.
 */
...
void Chip::GenerateBlock3( Bitu total, Bit32s* output  ) {
	while ( total > 0 ) {
		...
		int count = 0;
		for( Channel* ch = chan; ch < chan + 2; ) { // from chan + 18
			count++;
			ch = (ch->*(ch->synthHandler))( this, samples, output );
		}
		total -= samples;
...
151309648,151309648 };
	...
	{
		static const Bit32u precalculated_table[62] =
{2078,2607,3117,3584,4156,5213,6233,7168,8311,10426,12466,14336,16621,20853,
