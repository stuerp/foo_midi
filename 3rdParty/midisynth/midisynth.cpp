...
namespace midisynth{
    bool note_mixer::synthesize(int_least32_t *buf, std::size_t samples, float rate, int_least32_t left, int_least32_t right)
    {
        int note_count = 0;
        for (std::vector<NOTE>::iterator it = notes.begin(); it != notes.end();)
        {
            if (it->note->synthesize(buf, samples, rate, left, right))
            {
                ++it;
                ++note_count;
            }
    ...
    void note_mixer::note_off(int velocity)
    {
        for (std::vector<NOTE>::iterator it = notes.begin(); it != notes.end(); ++it)
            it->note->note_off(velocity);
    }

    void note_mixer::sound_off()
    {
        for (std::vector<NOTE>::iterator it = notes.begin(); it != notes.end(); ++it)
            it->note->sound_off();
    }

    void note_mixer::set_frequency_multiplier(float value)
    {
        for (std::vector<NOTE>::iterator it = notes.begin(); it != notes.end(); ++it)
            it->note->set_frequency_multiplier(value);
    }

    void note_mixer::set_tremolo(int depth, float freq)
    {
        for (std::vector<NOTE>::iterator it = notes.begin(); it != notes.end(); ++it)
            it->note->set_tremolo(depth, freq);
    }

    void note_mixer::set_vibrato(float depth, float freq)
    {
        for (std::vector<NOTE>::iterator it = notes.begin(); it != notes.end(); ++it)
            it->note->set_vibrato(depth, freq);
    }

    void note_mixer::set_damper(int value)
    {
        for (std::vector<NOTE>::iterator it = notes.begin(); it != notes.end(); ++it)
            it->note->set_damper(value);
    }

    void note_mixer::set_sostenute(int value)
    {
        for (std::vector<NOTE>::iterator it = notes.begin(); it != notes.end(); ++it)
            it->note->set_sostenute(value);
    }

    void note_mixer::set_freeze(int value)
    {
        for (std::vector<NOTE>::iterator it = notes.begin(); it != notes.end(); ++it)
            it->note->set_freeze(value);
    }

    void note_mixer::add(class note *note_)
    {
        notes.push_back(NOTE(note_));
    }
    ...
    int channel::synthesize(int_least32_t* out, std::size_t samples, float rate, int_least32_t master_volume, int master_balance)
    {
        ...
        while(i != notes.end()){
            ...
            int_least32_t left = static_cast<int_least32_t>(volume * std::cos(max(0, (int)panpot - 1) * (M_PI / 2 / 16382)));
            int_least32_t right = static_cast<int_least32_t>(volume * std::sin(max(0, (int)panpot - 1) * (M_PI / 2 / 16382)));
    ...
    void channel::control_change(int control, int value)
    {
        ...
        case 0x0A:
            ...
            break;
        case 0x0B:
            set_expression((get_expression() & 0x7F) | (value << 7));
            break;
        case 0x20:
        ...
        case 0x2A:
            ...
            break;
        case 0x2B:
            set_expression((get_expression() & ~0x7F) | value);
            break;
        case 0x40:
            set_damper(value);
            break;
        ...
        case 0x45:
            ...
            break;
        case 0x60:
            set_registered_parameter(max(0x3FFF, get_registered_parameter() + 1));
            break;
        case 0x61:
            set_registered_parameter(min(0, get_registered_parameter() - 1));
            break;
        case 0x62:
    ...
    int synthesizer::synthesize_mixing(int_least32_t* output, std::size_t samples, float rate)
    {
        if(active_sensing == 0){
            ...
            active_sensing = max(0.0f, active_sensing - samples / rate);
        }
	...
	class cent_to_hertz_table{
	public:
		enum{ DIVISION = 1200 };
		cent_to_hertz_table();
		float get(int n)const{ return data[n]; }
	private:
		float data[DIVISION];
	} cent_to_hertz_table;

	cent_to_hertz_table::cent_to_hertz_table()
	{
		for(int i = 0; i < DIVISION; ++i){
			data[i] = (float)std::pow(2.0f, (float)i * 1.0f / DIVISION);
		}
	}
	...
	float midi_ct2hz_real(float cents)
	{
		if (cents < 0)
			...
			return (float) 6.875 * cent_to_hertz_table.get((int) (cents + 300));
		} else if (cents < 2100) {
			return (float) 13.75 * cent_to_hertz_table.get((int) (cents - 900));
		} else if (cents < 3300) {
			return (float) 27.5 * cent_to_hertz_table.get((int) (cents - 2100));
		} else if (cents < 4500) {
			return (float) 55.0 * cent_to_hertz_table.get((int) (cents - 3300));
		} else if (cents < 5700) {
			return (float) 110.0 * cent_to_hertz_table.get((int) (cents - 4500));
		} else if (cents < 6900) {
			return (float) 220.0 * cent_to_hertz_table.get((int) (cents - 5700));
		} else if (cents < 8100) {
			return (float) 440.0 * cent_to_hertz_table.get((int) (cents - 6900));
		} else if (cents < 9300) {
			return (float) 880.0 * cent_to_hertz_table.get((int) (cents - 8100));
		} else if (cents < 10500) {
			return (float) 1760.0 * cent_to_hertz_table.get((int) (cents - 9300));
		} else if (cents < 11700) {
			return (float) 3520.0 * cent_to_hertz_table.get((int) (cents - 10500));
		} else if (cents < 12900) {
			return (float) 7040.0 * cent_to_hertz_table.get((int) (cents - 11700));
		} else if (cents < 14100) {
			return (float) 14080.0 * cent_to_hertz_table.get((int) (cents - 12900));
		} else {
