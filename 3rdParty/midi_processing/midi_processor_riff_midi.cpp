    ...
    while (it != body_end)
    {
        ...
        else if (it_equal(it, "DISP", 4))
        {
            ...
            if (type == 1)
            {
                ...
                meta_data.AddItem(midi_meta_data_item(0, "display_name", (const char *) &extra_buffer[0]));
            }
            it += 8 + chunk_size;
            if (chunk_size & 1 && it != body_end) ++it;
        }
            ...
            if (it_equal(it + 8, "INFO", 4))
            {
                if (!found_info)
                {
                    ...
                    while (it != chunk_end)
                    {
                        ...
                        it += 8 + field_size;
                        meta_data.AddItem(midi_meta_data_item(0, field.c_str(), (const char *) &extra_buffer[0]));
