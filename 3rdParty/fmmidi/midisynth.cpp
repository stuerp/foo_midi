...
namespace midisynth
{
    ...
    void channel::note_on(int note, int velocity)
    {
        ...
        if (velocity)
        {
            ...
            if (p)
            {
                ...
                if (modulation_depth)
                {
                    float depth = (float)((double) modulation_depth * modulation_depth_range / (16383.f * 128.f));
    ...
    {
        float value = (float) (master_frequency_multiplier * std::pow(2, (coarse_tuning - 8192) / (128.0 * 12.0) + (fine_tuning - 8192) / (8192.0 * 12.0) +
                               static_cast<double>(pitch_bend - 8192) * pitch_bend_sensitivity / (8192.f * 128.f * 12.f)));
    ...
    {
        float value = (float) std::pow(2, (master_coarse_tuning - 8192) / (128.f * 100.f * 12.f) + (master_fine_tuning - 8192) / (8192.f * 100.f * 12.f));
