...
EMIDIPlayer::EMIDIPlayer() : MIDIPlayer()
{
	bInitialized = false;
	synth_mode = mode_gm;
}
...
bool EMIDIPlayer::startup()
{
	...
	for ( unsigned i = 0; i < 8; ++i )
	{
		if ( i & 1 ) mModule[ i ].AttachDevice( new dsa::CSccDevice( uSampleRate, 2 ) );
		else         mModule[ i ].AttachDevice( new dsa::COpllDevice( uSampleRate, 2 ) );
		mModule[ i ].Reset();
	}
